# Zero-to-Low-Cost Deployment Guide: eCRM + emailVoice
...
| **emailVoice Web** | Vercel | Free | Same free tier |
| **emailVoice API + Workers** | Railway | Free ($5 credit/mo) | ~2–3 months free |
| **PostgreSQL (eCRM)** | Supabase | Free | 500 MB, 50k MAU |
| **PostgreSQL (emailVoice)** | Railway | Free | Included in free tier |
| **Redis** | Railway | Free | Included; used by emailVoice workers |
...
## Part 1: Deploy eCRM (Next.js + Supabase)

### Step 1.1: Prepare Supabase Database
...
3. Once created, go to **Settings → Database** → Copy `Connection string` (Psql):
   ```
   postgresql://postgres:[YOUR_PASSWORD]@[HOST]:[PORT]/postgres
   ```
4. Create `.env.local` in eCRM root:
   ```
   DATABASE_URL="postgresql://postgres:[PASSWORD]@[HOST]:5432/postgres"
   POSTGRES_PRISMA_URL="postgresql://postgres:[PASSWORD]@[HOST]:5432/postgres?schema=public"
...
5. **Environment Variables**: Add from your `.env.local`:
   ```
   DATABASE_URL = postgresql://postgres:[PASSWORD]@[HOST]:5432/postgres
   ...
   ```
6. Click **Deploy**
7. After ~5–10 min, your app will be live at `https://ecrm-[random].vercel.app`
...
## Part 2: Deploy emailVoice (FastAPI + Node.js + PostgreSQL + Redis)

### Step 2.1: Prepare Docker Images

emailVoice already has `apps/api/Dockerfile`. Ensure it exists and is production-ready:
...
**Database (PostgreSQL)**:
```
POSTGRES_USER=emailvoice
POSTGRES_PASSWORD=[GENERATE_STRONG]
POSTGRES_DB=emailvoice
```
...
### Step 3.1: Update eCRM Environment for emailVoice API

If eCRM needs to call emailVoice API, add to Vercel:

**Vercel → Project Settings → Environment Variables**:
```
NEXT_PUBLIC_EMAILVOICE_API_URL=https://[railway-api-url]
```

### Step 3.2: Update emailVoice Web to Call API

Already configured in Railway if you set `VITE_API_URL` correctly.

### Step 3.3: Enable CORS Between Domains

**emailVoice API** (`apps/api/app/main.py`):
```python
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://emailvoice-[random].railway.app",
    ],
    allow_methods=["*"],
    allow_headers=["*"],
)
...
2. **emailVoice API**: Railway → Settings → Domains → Add `api.yourdomain.com`
...
- emailVoice: When >100k requests/month or DB >500 MB
...
### emailVoice API returning 500 errors
```bash
...
4. Create Railway project → auto-deploys emailVoice
