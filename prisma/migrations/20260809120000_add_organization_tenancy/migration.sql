...
-- The concrete ownership matrix from architecture section 4. The dynamic DDL
-- is repeat-safe so a rehearsal can re-execute this SQL without duplicate DDL.
DO $$
DECLARE
  tenant_table TEXT;
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format(
      ...
      tenant_table || '_organizationId_idx',
      tenant_table
    );

    constraint_name := left(tenant_table || '_organizationId_fkey', 63);
    IF NOT EXISTS (
      SELECT 1
      FROM pg_constraint
...
-- Nullable compound uniqueness is valid in PostgreSQL. Replace only the old
-- global external-key index; no rows or business columns are dropped.
...
SET "legalName" = EXCLUDED."legalName",
    "displayName" = EXCLUDED."displayName",
    "status" = EXCLUDED."status",
    "deploymentRegion" = EXCLUDED."deploymentRegion",
...
SET "currency" = EXCLUDED."currency",
    ...
    "defaultCountry" = EXCLUDED."defaultCountry",
    "taxConfiguration" = EXCLUDED."taxConfiguration",
    "invoiceConfiguration" = EXCLUDED."invoiceConfiguration",
    "paymentConfiguration" = EXCLUDED."paymentConfiguration",
    "numberingRules" = EXCLUDED."numberingRules",
    "retentionPolicy" = EXCLUDED."retentionPolicy",
    "financialClosePolicy" = EXCLUDED."financialClosePolicy",
    "enabledModules" = EXCLUDED."enabledModules",
    "operationalLimits" = EXCLUDED."operationalLimits",
    "updatedAt" = CURRENT_TIMESTAMP;
...
SET "productName" = EXCLUDED."productName",
    ...
    "documentFooter" = EXCLUDED."documentFooter",
    "updatedAt" = CURRENT_TIMESTAMP;
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'SELECT COUNT(*) FROM %I child JOIN %I parent ON parent."id" = child.%I WHERE child.%I IS NOT NULL AND child."organizationId" IS DISTINCT FROM parent."organizationId"',
